...
namespace CustomDev.Security.Cryptography
{
    public static class AesCryptography
    {
        ...
        public static string EncryptWithAesBase64(byte[] plainContent, byte[] key)
        {
            byte[] encodedContent = EncryptWithAes(plainContent, key);
            string base64 = Convert.ToBase64String(encodedContent);
            return base64;
        }

        public static byte[] DecryptWithAesBase64(string encodedBase64Content, byte[] key)
        {
            byte[] encodedContent = Convert.FromBase64String(encodedBase64Content);
            byte[] content = DecryptWithAes(encodedContent, key);
            return content;
        }

        public static string EncryptTextWithAesBase64(string plainText, byte[] key)
        {
            byte[] content = Encoding.UTF8.GetBytes(plainText);
            string base64 = EncryptWithAesBase64(content, key);
            return base64;
        }

        public static string DecryptTextWithAesBase64(string encodedBase64Content, byte[] key)
        {
            byte[] content = DecryptWithAesBase64(encodedBase64Content, key);
